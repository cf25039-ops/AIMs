...
async function seedHierarchy() {
  ...
  for (const project of projects) {
    ...
    // 1. Contract
    let { data: contracts, error: cErr } = await supabase.from('contracts').select('*').eq('project_id', project.id);
    if (cErr) throw cErr;
    let contract = contracts && contracts.length > 0 ? contracts[0] : null;
    
    if (!contract) {
      console.log("  -> Creating default contract...");
      const { data: newContract, error: insertCErr } = await supabase.from('contracts').insert({
    ...
    // 2. Region
    let { data: regions, error: rErr } = await supabase.from('regions').select('*').eq('contract_id', contract.id);
    if (rErr) throw rErr;
    let region = regions && regions.length > 0 ? regions[0] : null;

    if (!region) {
      console.log("  -> Creating default region...");
      const { data: newRegion, error: insertRErr } = await supabase.from('regions').insert({
    ...
    // 3. State
    let { data: states, error: sErr } = await supabase.from('states').select('*').eq('region_id', region.id);
    if (sErr) throw sErr;
    let state = states && states.length > 0 ? states[0] : null;

    if (!state) {
      console.log("  -> Creating default state...");
      const { data: newState, error: insertSErr } = await supabase.from('states').insert({
        region_id: region.id,
    ...
    // 4. Facility
    let { data: facilities, error: fErr } = await supabase.from('facilities').select('*').eq('state_id', state.id);
    if (fErr) throw fErr;
    let facility = facilities && facilities.length > 0 ? facilities[0] : null;

    if (!facility) {
      console.log("  -> Creating default facility...");
      const { data: newFacility, error: insertFErr } = await supabase.from('facilities').insert({
        state_id: state.id,
    ...
    // 5. Department
    let { data: depts, error: dErr } = await supabase.from('departments').select('*').eq('facility_id', facility.id);
    if (dErr) throw dErr;
    let dept = depts && depts.length > 0 ? depts[0] : null;

    if (!dept) {
      console.log("  -> Creating default department...");
      const { data: newDept, error: insertDErr } = await supabase.from('departments').insert({
