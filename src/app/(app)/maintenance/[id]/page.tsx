...
const statusOptions = [
  "open", "assigned", "investigation", "in_repair",
  "vendor_escalation", "testing", "resolved", "closed",
];

const statusIcons: Record<string, any> = {
  open: AlertTriangle,
  assigned: User,
  investigation: Wrench,
  in_repair: Wrench,
  vendor_escalation: AlertTriangle,
  ...
  if (!ticket) {
    return (
      <div className="flex flex-col items-center justify-center h-96 gap-4">
        ...
        <Link href="/maintenance"><Button variant="outline">Back to Maintenance</Button></Link>
      </div>
    );
  }

  const sla = ticket.status !== "resolved" && ticket.status !== "closed"
        ...
        <Card className="glass-card">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {sla && (
                <span className={cn(
                  ...
                  sla.isBreached ? "text-rose-600 bg-rose-500/10 border-rose-500/20" :
                  sla.isCritical ? "text-amber-600 bg-amber-500/10 border-amber-500/20" :
          ...
          <CardContent className="pt-4 space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold block">Asset</span>
                <Link href={`/assets/${ticket.hardware?.id}`} className="text-foreground hover:text-primary font-medium">
                  {ticket.hardware?.brand} {ticket.hardware?.model}
                </Link>
                <p className="text-xs text-muted-foreground">{ticket.hardware?.asset_tag}</p>
              </div>
              <div>
                <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold block">Reported</span>
                <span className="text-foreground">{new Date(ticket.created_at).toLocaleDateString()}</span>
              </div>
              <div>
                <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold block">Assigned To</span>
                <span className="text-foreground">{ticket.assigned_user?.full_name || "Unassigned"}</span>
              </div>
              <div>
                <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold block">Severity</span>
            ...
            <Card className="glass-card">
              ...
              <CardContent className="space-y-3">
                {canManage && !isAddingNote && (
                  <Button variant="outline" size="sm" onClick={() => setIsAddingNote(true)} className="gap-1.5 w-full">
                ...
                {isAddingNote && (
                  <div className="space-y-2 rounded-xl border border-primary/20 bg-primary/5 p-3">
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" onClick={() => addNoteMutation.mutate(newNote)} disabled={!newNote || addNoteMutation.isPending} className="gap-1 text-xs">
                        {addNoteMutation.isPending && <Loader2 className="h-3 w-3 animate-spin" />}
                        Save Note
                      </Button>
                      <Button size="sm" variant="outline" onClick={() => setIsAddingNote(false)} className="text-xs">Cancel</Button>
                    </div>
                  </div>
                )}
                    ...
                    <div key={log.id} className="flex gap-3 border-l-2 border-primary/20 pl-4 py-1">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-semibold text-foreground">{log.created_by?.full_name || "Unknown"}</span>
                ...
                {ticket.attachments?.length > 0 ? (
                  <div className="space-y-2">
                    {ticket.attachments.map((att: any) => (
                      <a key={att.id} href={att.url} target="_blank" rel="noopener noreferrer" className="block text-sm text-primary hover:underline truncate">
                        {att.url.split("/").pop()}
                      </a>
                    ))}
                  </div>
