      ...
      <FadeIn className="flex flex-col gap-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Manage spare parts inventory and purchase requests</p>
          </div>
        </div>
      </FadeIn>
      ...
      <FadeIn delay={0.1}>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          <Card className="glass-card">
            ...
            <CardContent>
              {loadingInventory ? <Loader2 className="h-4 w-4 animate-spin" /> : (
          ...
          <Card className="glass-card border-l-4 border-l-rose-500">
            ...
            <CardContent>
              {loadingInventory ? <Loader2 className="h-4 w-4 animate-spin" /> : (
                <div className="text-2xl font-bold text-rose-600 dark:text-rose-400">{lowStockItems.length}</div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="glass-card border-l-4 border-l-amber-500">
            ...
            <CardContent>
              {loadingRequests ? <Loader2 className="h-4 w-4 animate-spin" /> : (
              ...
              <CardContent>
                {loadingInventory ? (
                  ...
                  <EmptyState
                    ...
                    action={{ label: "Add Inventory Item", onClick: () => alert("Add inventory form here") }}
                  />
                ) : (
                          ...
                          return (
                            <TableRow key={item.id} className={isLowStock ? "bg-destructive/5" : ""}>
                              ...
                              <TableCell className="text-right font-medium">
                                <span className={isLowStock ? "text-rose-600 dark:text-rose-400" : ""}>{item.quantity}</span>
                                <span className="text-xs text-muted-foreground ml-1">/ {item.min_quantity}</span>
                              </TableCell>
                              <TableCell className="text-center">
                                {isLowStock ? (
                                  <Badge variant={"destructive" as any} className="uppercase text-[10px]">Restock</Badge>
                                ) : (
                                  <Badge variant={"success" as any} className="uppercase text-[10px]">Healthy</Badge>
                                )}
                              </TableCell>
                            </TableRow>
                          )
                ...
                {loadingRequests ? (
                  ...
                  <div className="space-y-4">
                    {requests.slice(0, 5).map((req: any) => (
                      <div key={req.id} className="flex flex-col gap-1 p-3 rounded-lg border bg-card text-sm">
                        <div className="flex justify-between items-start">
                          <p className="font-medium text-foreground">{req.item_name}</p>
                          <Badge 
                            variant={req.status === 'approved' ? 'success' as any : req.status === 'rejected' ? 'destructive' : 'warning' as any} 
