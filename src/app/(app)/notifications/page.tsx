...
export default function NotificationsPage() {
  ...
  const getNotificationIcon = (title: string) => {
    ...
    if (t.includes("alert") || t.includes("urgent") || t.includes("critical"))
      return <BellRing className="h-5 w-5 text-rose-500" />;
    if (t.includes("success") || t.includes("resolved"))
    ...
    return (
      <div className="divide-y">
        {list.map((n: any) => (
          <div
            key={n.id}
            className={`p-4 hover:bg-muted/30 transition-colors flex gap-4 ${!n.read_at ? "bg-primary/5" : ""}`}
          >
            <div className="mt-1 flex-shrink-0">{getNotificationIcon(n.title)}</div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Manage your system alerts and automated messages
            </p>
          </div>
        </div>
