      ...
      {isAdding && (
        <div className="rounded-xl border border-primary/20 bg-primary/5 p-3 space-y-2">
          ...
          <Input
            ...
            placeholder="Icon key (optional, e.g. monitor)"
            className="h-9 text-sm"
          />
          ...
          {types.map((type) => (
            <TypeRow
              ...
              onDelete={() => {
                if (
                  confirm(
                    `Delete "${type.name}"? Hardware linked to this type will have their category unset.`,
                  )
                ) {
      ...
      {isEditing ? (
        <div className="flex-1 space-y-1.5">
          ...
          <div className="flex gap-1.5">
            <Button
              size="sm"
              onClick={() => onSave({ name, description: desc, icon })}
              className="h-7 text-[10px]"
            >
              Save
            </Button>
            <Button size="sm" variant="outline" onClick={onCancelEdit} className="h-7 text-[10px]">
              Cancel
            </Button>
          </div>
        </div>
